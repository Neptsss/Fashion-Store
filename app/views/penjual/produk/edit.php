    ...
    <div class="table-container" style="padding: 30px; max-width: 800px;">
        <form action="<?= BASE_URL; ?>/dashboard/products/update" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama" class="form-control" value="<?= $data['produk']['nama'] ?>" required>
            </div>
            <div class="form-group">
                <label>Kategori</label>
                <select name="kategori_id" class="form-control" required>
                    <?php foreach ($data['kategori'] as $k): ?>
                    <option value="<?= $k['id'] ?>" <?= $k['id'] == $data['produk']['kategori_id'] ? 'selected' : '' ?>><?= $k['nama'] ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                <label>Ukuran</label>
                <input type="text" name="ukuran" class="form-control" value="<?= $data['produk']['ukuran'] ?>">
            </div>
            <div class="form-group">
                ...
                <div style="margin-top: 10px;">
                    <img src="<?= BASE_URL; ?>/images/<?= $data['produk']['foto'] ?>" alt="Gambar Saat Ini" style="height: 100px; border-radius: 8px;">
                </div>
                <?php endif; ?>
            </div>
            <div class="form-group">
                <label>Deskripsi</label>
                <textarea name="deskripsi" class="form-control" rows="5"><?= $data['produk']['deskripsi'] ?></textarea>
            </div>
